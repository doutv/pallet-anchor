...
fn unsell_anchor() {
    new_test_ext().execute_with(|| {
        ...
        System::set_block_number(System::block_number() + step);
        assert_ok!(Anchor::unsell_anchor(account_a.clone(), key.clone()));
        assert_eq!(Anchor::selling(&key), None);
        assert_eq!(Anchor::owner(&key), Some((id_a, start_block)));
    });
}
...
fn buy_anchor() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Anchor::buy_anchor(account_c.clone(), key.clone()));
        ...
        //9. done!
        assert_ok!(Anchor::buy_anchor(account_b.clone(), key.clone()));
        assert_eq!(Anchor::selling(&key), None);
